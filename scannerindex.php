...
if (!empty($_POST["ID"]))
    {
        ...
        elseif($check==1)
        {
            $startquery = "SELECT checkintime FROM tbl_checkinout WHERE regno = '" . $user_id . "' and used is null";
            $result3 =  $conn->query($checkquery);
            $check = 0;
            while($row = $result3->fetch_assoc())
            {
            ...
            $updatequery = "update tbl_checkinout set checkouttime='".$end."' ,used=".$mins." WHERE (regno = '" . $user_id . "' and used is null)";
        ...
        else
        {
            ...
            $time= date("H:i:s");
            $date= date("Y-m-d");
            $updatequery = "insert into tbl_checkinout(regno,name,dept,year,design,date,checkintime) values('".$user_id."','".$name."','".$dept."','".$year."','".$desig."','".$date."','".$time."')";
            if(mysqli_query($conn, $updatequery))
            {
                echo "<meta http-equiv='refresh' content='0'>";
            }
